...
function formatStats(states) {
  ...
  states.forEach(state => {
    ...
    const lastActivity = state.last_activity ? escape(new Date(state.last_activity).toLocaleString('id-ID')) : 'N/A';
    totalPeanut += peanut; totalVcus += vcus; totalTasks += tasks;
...
bot.onText(/\/status/, (msg) => {
  ...
  states.forEach(state => {
    ...
    const label = diffMin <= 5 ? 'Aktif' : diffMin <= 30 ? `Lambat \\(${diffMin}m\\)` : `Mati \\(${diffMin}m\\)`;
...
bot.onText(/\/setcompute/, (msg) => {
  ...
  bot.sendMessage(CHAT_ID,
    ...
    { parse_mode: 'MarkdownV2', reply_markup: { inline_keyboard: [[{ text: '🖥️ CPU', callback_data: 'compute_CPU' }, { text: '🎮 GPU', callback_data: 'compute_GPU' }]] } }
  );
});
