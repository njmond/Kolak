...
async function registerAgent(accountId, publicKey, computeCapability, maxVcus, wallet, maxRetries = 10) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (resp.status === 200) {
        ...
        // Delay makin panjang tiap gagal, max 30 detik
        const delay = Math.min(Math.pow(2, attempt) * 1000, 30000);
        if (attempt < maxRetries) {
          log(`⏳ Retrying in ${delay/1000}s...`, accountId);
          await sleep(delay);
        }
      }
    } catch (err) {
  ...
  log(`⚠️ All registration attempts failed, will keep trying on submit...`, accountId, "WARN");
  return { success: false, data: null };
}

async function getCurrentTask() {
  try {
    const resp = await fetch(`${BASE_URL}/tasks/current`, { method: 'GET', signal: AbortSignal.timeout(60000) });
...
async function runAccountMiner(account, config, license) {
  ...
  // Prioritas: env PEANUT_PRIVATE_KEY_1 > accounts.json > state > generate baru
  // Env variable paling persisten di Railway (filesystem ephemeral)
  const envKeyName = `PEANUT_PRIVATE_KEY_${accountId.replace(/\D/g, '') || '1'}`;
  let privateKey = process.env[envKeyName] || account.private_key || state.private_key || null;
  let publicKey = null;
  ...
  if (!privateKey) {
    ...
    log(`💾 Keypair generated & saved`, accountId);
    log(`💡 Tip: Set ${envKeyName}=<key> di Railway Variables agar keypair persisten!`, accountId, "WARN");
  } else {
    try {
