...
bot.onText(/\/help/, (msg) => {
  ...
  sendMessage(
    ...
    '/listaccounts \\- Lihat semua akun\n' +
    '/removeaccount \\- Hapus akun\n' +
    '/getkey \\- Ambil private key agent untuk Railway\n' +
...
// /getkey - ambil private key agent untuk Railway Variables
bot.onText(/\/getkey/, (msg) => {
  if (!isAuthorized(msg)) return;
  const config = loadConfig();
  if (config.accounts.length === 0) {
    return sendMessage('❌ Belum ada akun\. Miner belum pernah jalan\.');
  }
  let hasKey = false;
  let text = '🔑 *Private Key Agent*\n\n';
  text += '⚠️ _Ini BUKAN private key wallet kamu\._ \n';
  text += '_Aman disimpan di Railway Variables\._ \n\n';
  config.accounts.forEach(acc => {
    if (acc.private_key) {
      hasKey = true;
      const num = acc.id.replace(/\D/g, '') || '1';
      text += `👤 *${escape(acc.id)}*\n`;
      text += `Tambahkan ke Railway Variables:\n`;
      text += `Name: \`PEANUT\_PRIVATE\_KEY\_${escape(num)}\`\n`;
      text += `Value: \`${escape(acc.private_key)}\`\n\n`;
    }
  });
  if (!hasKey) {
    return sendMessage('⏳ Keypair belum terbuat\. Tunggu miner jalan dulu lalu coba lagi\.');
  }
  sendMessage(text);
});
