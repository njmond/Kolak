...
// ==============================
// PATCH: Railway non-interactive support
// Tunggu .license dari Telegram bot jika tidak ada TTY
// ==============================
export async function verifyLicense({ appName = LicenseConfig.APP_NAME, checkUrl = LicenseConfig.CHECK_CODE_URL, licenseFile = LicenseConfig.LICENSE_FILE, hintMessage } = {}) {
  ...
  // Railway / non-interactive: tunggu file .license dari Telegram bot
...
// ==============================
// API FUNCTIONS (timeout 3s sesuai repo resmi)
// ==============================
...
async function submitProof(accountId, taskId, solution, signature, computeTimeMs, maxRetries = 3) {
  return withRetry(async () => {
    ...
    // Handle duplicate submission sebagai sukses
